    ...
    def asignar(self, proceso, tamanio_requerido):
        for i in range(self.tamanio - tamanio_requerido + 1):
            ...
            if all(self.memoria[i + j] is None for j in range(tamanio_requerido)):
                ...
                return True
        print(f"{proceso} no pudo asignar memoria. No hay suficiente espacio contiguo.")
        return False
...
def ejecutar_proceso(nombre, memoria):
    ...
    time.sleep(random.randint(1, 2))  # Simulamos tiempo entre procesos
...
def gestionar_memoria():
    ...
    for _ in range(5):
        ...
        ejecutar_proceso(proceso_nombre, memoria)  # Ejecutar el proceso
        memoria.mostrar_memoria()  # Mostrar el estado de la memoria
